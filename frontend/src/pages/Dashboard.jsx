      ...
      <div className="grid gap-6 xl:grid-cols-[1.7fr_0.9fr]">
        <Card padded={false} className="overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-6">
            <div>
              ...
              <p className="text-sm text-slate-500">Latest team activity and status updates.</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="4" className="px-5 py-8 sm:px-6">
                      <Loader label="Loading recent employees..." />
                    </td>
                  </tr>
                ) : error ? (
                  <tr>
                    <td colSpan="4" className="px-5 py-8 text-center text-sm text-rose-600 sm:px-6">
                      {error}
                    </td>
                  </tr>
                ) : recentEmployees.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-5 py-8 text-center text-sm text-slate-500 sm:px-6">
                      No recent employees found.
                    </td>
                  </tr>
                ) : (
                  recentEmployees.map((employee) => (
                    <tr key={employee._id || employee.email} className="border-t border-slate-200/80">
                      ...
                      <td className="px-5 py-4 text-slate-600 sm:px-6">{employee.department}</td>
                      <td className="px-5 py-4 text-slate-600 sm:px-6">{employee.position}</td>
                      <td className="px-5 py-4 sm:px-6">
        ...
        <div className="space-y-6">
          <Card>
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-slate-900">Stay ahead</h3>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {quickActions.map((item) => (
                ...
                >
                  ...
                  <Link to="/employees" className="text-indigo-600 transition hover:text-indigo-700">
          ...
          <Card className="bg-gradient-to-br from-indigo-600 to-violet-600 text-white">
            <p className="text-sm font-medium text-indigo-100">Team health</p>
            <h3 className="mt-2 text-xl font-semibold">82% engagement score</h3>
            <p className="mt-2 text-sm text-indigo-100">
              New hires are moving faster with the updated onboarding experience.
            </p>
            <Button to="/employees" variant="secondary" className="mt-5 bg-white text-indigo-700 hover:bg-indigo-50">
